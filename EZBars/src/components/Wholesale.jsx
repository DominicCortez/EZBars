...
const onSubmit = (data) => {
    axios.post("http://localhost:3001/products", data, newItemSupplier).then((response) => {
...
const editItem = (option,id) => {
  if (option === "itemsupplier"){
    ...
    axios.put("http://localhost:3001/products/itemsupplier",{newItemSupplier : newItemSupplier, id : id})
    window.location.reload(false)
  }else if (option === "itemname"){
    let newItemName = prompt("Enter new Item Name: ")
    axios.put("http://localhost:3001/products/itemname",{newItemName : newItemName, id : id})
    window.location.reload(false)
  }
  else if (option === "itemdescription"){
    let newItemDescription = prompt("Enter new Item Description: ")
    axios.put("http://localhost:3001/products/itemdescription",{newItemDescription : newItemDescription, id : id})
    window.location.reload(false)
  }else{
    let newItemPrice = prompt("Enter new Item Price: ")
    axios.put("http://localhost:3001/products/itemprice",{newItemPrice : newItemPrice, id : id})
    window.location.reload(false)
  }
}
